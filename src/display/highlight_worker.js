  ...
  if (doc.highlightFrontier < cm.display.viewTo) {
    ...
    doc.iter(context.line, Math.min(doc.first + doc.size, cm.display.viewTo + 500), line => {
      if (context.line >= cm.display.viewFrom) { // Visible
        ...
        line.tokens = highlighted.tokens
        ...
        if (
          !oldTokens 
          || oldTokens.length != line.tokens.length 
          || oldCls != newCls && (
          ...
          for (let i = 0; !ischange && i < oldTokens.length; ++i) {
            ischange = oldTokens[i].token != line.tokens[i].token
          }
        }
